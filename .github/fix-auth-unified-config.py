...
runtime_method = """  _loadRuntimeConfig: async function() {
    try {
...
new_ambiguous = """        if (ambiguousExisting) {
          return {
            success: false,
            existing: true,
            signupState: true,
...
if old_ambiguous not in auth:
    raise SystemExit('signup result anchor not found')
auth = auth.replace(old_ambiguous, new_ambiguous, 1)
...
old_lib_head = """const crypto = require('crypto');
...
new_lib_head = f"""const crypto = require('crypto');

const FALLBACK_SUPABASE_URL = 'https://ebdnkgfilnvfkkdvqrzu.supabase.co';
const FALLBACK_SUPABASE_ANON_KEY = '{fallback_key}';
...
const SUPABASE_SERVICE_ROLE_KEY = (process.env.SUPABASE_SERVICE_ROLE_KEY || '').trim();"""
if old_lib_head not in lib:
    raise SystemExit('_lib config anchor not found')
lib = lib.replace(old_lib_head, new_lib_head, 1)
