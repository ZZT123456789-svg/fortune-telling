...
const BaziModule = {
  ...
  _baziA(result,gender) {
    ...
    const prof={'金':`日主${result.dayPillar[0]}（${dm}命），金主义——刚毅果断、重情重义。${s==='偏强'?'命局金气旺盛如百炼精钢，领袖气质，意志坚强。'：s==='偏弱'?'金气稍弱如待琢璞玉，外柔内刚，愈炼愈精。':'金气中和，刚柔并济，难得的好格局。'}`,'木':`日主${result.dayPillar[0]}（${dm}命），木主仁——仁厚温和、生命力旺。${s==='偏强'?'木气蓬勃如参天大树，志向远大，栋梁之才。':'木气如精致盆景，心思细腻，大器晚成。':'木气舒展有度，既有生长力又有柔韧性。'}`, '水':`日主${result.dayPillar[0]}（${dm}命），水主智——聪慧灵活、适应力强。${s==='偏强'?'水势浩荡如大江奔流，格局宏大，战略眼光。':'水气如清泉细流，直觉力强，心思敏感。':'水气恰到好处，智慧而不狡猾。'}`, '火':`日主${result.dayPillar[0]}（${dm}命），火主礼——热情奔放、充满活力。${s==='偏强'?'火势熊熊如日中天，气场强大，天生领袖。':'火气如温暖烛光，低调有魅力。':'火气适中，热情有度。'}`, '土':`日主${result.dayPillar[0]}（${dm}命），土主信——稳重诚信、包容大度。${s==='偏强'?'土气厚重如泰山巍峨，定海神针。':'土气如精致陶瓷，温润细腻，大器晚成。':'土气恰到好处，稳重有原则，厚福之命。'}`};
  ...
  _strengthA(dm,s,wx,yS,ms) {
    ...
    let a=`格局「${s}」。${s==='偏强'?'日主能量充足，精力旺盛，抗压能力强。但过强则刚而易折，需适当收敛锋芒。'：s==='偏弱'?'日主能量稍弱，心思细腻敏感。需通过后天学习积累增强自身，大器晚成。':'日主中和！五行流通平衡，能屈能伸，刚柔并济，是难得的平衡格局。'}`;
  ...
  _taoA(dm,s,gender) {
    ...
    let a=`根据日主${dm}，为您从《道德经》中择取以下智慧指引：\n`;
    chapters.forEach(c=>{a+=`\n📖 第${c.ch}章「${c.t}」：${c.e}`});
    a+=`\n\n老子云"${s==='偏强'?'弱者道之用'：s==='偏弱'?'天下之至柔驰骋天下之至坚':'道常无为而无不为'}”——${s==='偏强'?'您命局偏强，当学水之柔韧，以柔济刚。':'您命局偏弱，当知柔弱胜刚强，以智慧补力量之不足。':'您命局中和，当效法道之自然，不妄为而万物自化。'}`;
    return a;
  },
  ...
  _wealthA(dm,wx,yS,ms) {
    ...
    a+=`\n\n财富节奏：${dm==='金'?'35-45岁黄金积累期':dm==='木'?'30岁前投资自己回报在下半场':dm==='水'?'财运流动性强，多元化配置最佳':dm==='火'?'赚钱能力强消费也大方，30%强制储蓄':dm==='土'?'稳健增长，40岁后财务安全感大幅提升'}。`;
    return a;
  },
